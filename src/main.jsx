...
function App() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setTank(v => Math.max(8, Math.min(96, v + (Math.random() - 0.55) * 1.2)))
      setRainTank(v => Math.max(5, Math.min(98, v + (Math.random() - 0.5) * 0.8)))
      setUsage(v => +(v + (valveOn ? Math.random() * 1.1 : 0)).toFixed(1))
      setQuality(v => Math.max(88, Math.min(96, +(v + (Math.random() - 0.5) * 0.4).toFixed(1))))
      setRainCollected(v => +(v + (Math.random() > 0.78 ? Math.random() * 0.6 : 0)).toFixed(1))
      setHistory(old => old.map((item, index) => index === old.length - 1 ? { ...item, usage: Math.max(0, item.usage + Math.round(Math.random() * 2 - 1)) } : item))
    }, 3500)
